					...
					<?php 
						...
						foreach ($pdo->query($sql) as $row) {
							...
							if ($row['countAssigns'] == 0)
								echo '<td>'. htmlspecialchars(trim($row['lname']) . ', ' . trim($row['fname']) . ' (' . substr($row['title'], 0, 1) . ') ') .' - UNASSIGNED</td>';
							else
								echo '<td>'. htmlspecialchars(trim($row['lname']) . ', ' . trim($row['fname']) . ' (' . substr($row['title'], 0, 1) . ') - '.$row['countAssigns']) . ' events</td>';
							echo '<td>'. htmlspecialchars($row['email']) . '</td>';
							echo '<td>'. htmlspecialchars($row['mobile']) . '</td>';
							echo '<td>'. htmlspecialchars($row['address'] . ' ' . $row['city'] . ' ' . $row['state'] . ' ' . $row['zipcode']) . '</td>';
							echo '<td width=250>';
							# always allow read
