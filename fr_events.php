				...
				<tbody>
					<?php 
						...
						foreach ($pdo->query($sql) as $row) {
							echo '<tr>';
							echo '<td>'. htmlspecialchars(Functions::dayMonthDate($row['event_date'])) . '</td>';
							echo '<td>'. htmlspecialchars(Functions::timeAmPm($row['event_time'])) . '</td>';
							echo '<td>'. htmlspecialchars($row['event_location']) . '</td>';
							if ($row['countAssigns']==0)
								echo '<td>'. htmlspecialchars($row['event_description']) . ' - UNSTAFFED </td>';
							else
								echo '<td>'. htmlspecialchars($row['event_description']) . ' (' . $row['countAssigns']. ' volunteers)' . '</td>';
							//echo '<td width=250>';
							echo '<td>';
