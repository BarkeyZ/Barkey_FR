					...
					<div class="controls">
						<?php
							...
							foreach ($pdo->query($sql) as $row) {
								if($row['id']==$person)
									echo htmlspecialchars("<option selected value='" . $row['id'] . " '> " . $row['lname'] . ', ' .$row['fname'] . "</option>");
								else
									echo htmlspecialchars("<option value='" . $row['id'] . " '> " . $row['lname'] . ', ' .$row['fname'] . "</option>");
							}
							echo "</select>";
						?>
				...
				<div class="control-group">
					...
					<div class="controls">
						<?php
							...
							foreach ($pdo->query($sql) as $row) {
								if($row['id']==$event) {
									echo "<option selected value='" . htmlspecialchars($row['id']) . " '> " . htmlspecialchars(Functions::dayMonthDate($row['event_date'])) . " (" . htmlspecialchars(Functions::timeAmPm($row['event_time'])) . ") - " . htmlspecialchars(trim($row['event_description'])) . " (" . htmlspecialchars(trim($row['event_location'])) . ") " . "</option>";
								}
								else {
									echo "<option value='" . $row['id'] . " '> " . htmlspecialchars(Functions::dayMonthDate($row['event_date']) . " (" . Functions::timeAmPm($row['event_time']) . ") - " . trim($row['event_description']) . " (" . trim($row['event_location']) . ") ") . "</option>";
								}
							}
							echo "</select>";
						?>
